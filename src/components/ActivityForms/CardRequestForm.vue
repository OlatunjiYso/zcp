<template>
  ...
  <div
    ...
    id="bulk-status-success"
  >
    <div class="app-modal-div success" style="text-align: center">
      <div>
        <div class="alert-icon"></div>
        <div class="alert-message">Request sent successfully</div>
        <div
          @click="resetState2"
  ...
  <div
    style="z-index: 999999; display: none"
    class="app-modal-overlay"
    id="bulk-status-failed"
  >
    <div class="app-modal-div success" style="text-align: center">
      <div>
        <div class="alert-icon failed"></div>
        <div class="alert-message" id="error-text">
          Operation Failed - Invalid File
        </div>
        <div
          @click="resetState3"
          style="margin-top: 30px; cursor: pointer"
  ...
  <div
    @click="isBulk = !isBulk"
    style="cursor: pointer; background: #c00; float: left"
    class="table-btn"
  >
  ...
  <form @submit.prevent="sendJson" v-if="isBulk">
    <label for="myfile"
      >Bulk Upload |
      <a
        href="https://res.cloudinary.com/vlog/raw/upload/v1634143152/DEBIT_BULK_SAMPLE.zip"
        download
        >Download Request Sample</a
      ></label
    >
    <input
      class="bulk-upload"
    ...
    />
    <br />
    <button v-show="isActiveBtn" type="submit" class="app-form-button">
      Submit
    </button>
  </form>
  <form @submit.prevent="sendRequest" v-else>
    <div class="form-flex">
      ...
      <div class="form-flex-col-3">
        ...
        >
          <option
            v-for="(result, index) in maritalStatus"
            :key="index"
            :value="result.code"
          >
            {{ result.name }}
      ...
      <div class="form-flex-col-3">
        ...
        >
          <option
            v-for="(result, index) in stateCities"
            :key="index"
            :value="result.code"
          >
            {{ result.name }}
      ...
      <div class="form-flex-col-3">
        <label class="login-label"
          >Legal ID Number<span style="color: red">*</span></label
        >
        <input
      ...
      <div class="form-flex-col-3">
        ...
        <select
          required
          v-model="form.idCardTypeCode"
          style="marginbottom: 30px"
          class="app-select w-select"
        >
          <option
            v-for="(result, index) in idCardType"
            :key="index"
            :value="result.code"
          >
            {{ result.description }}
      ...
      <div
        ...
        v-show="form.idCardTypeCode != '02' && form.idCardTypeCode != '04'"
      >
        <label class="login-label"
          >Expiry Date<span style="color: red">*</span></label
        >
        <input
          v-model="form.expiryDateOfDoc"
          :min="newSorted"
          type="date"
          class="app-text-field w-input"
          placeholder="Type Here"
      ...
      <div class="form-flex-col-3">
        <label class="login-label"
          >Name on Card<span style="color: red">*</span></label
        >
        <input
          @keydown="specialCharVal($event)"
      ...
      <div class="form-flex-col-3">
        <label class="login-label"
          >Socio Prof Code<span style="color: red">*</span></label
        >
        <select
          required
          v-model="form.socioProfCode"
          style="marginbottom: 30px"
          class="app-select w-select"
        >
          <option
            v-for="(result, index) in socioProf"
            :key="index"
            :value="result.socioCode"
          >
            {{ result.description }}
          </option>
        </select>
  ...
  methods: {
      ...
      reader.onload = function (event) {
        ...
        workbook.SheetNames.forEach(async function (sheetName) {
          ...
          const company = await axios.get(
            "https://acctgw.zenithbank.com/AcctAPIGateWay/CardportalSecurityAPi/api/companies/" +
              parseInt(user.companyId)
          );
          const companyProduct = await axios.get(
            "https://acctgw.zenithbank.com/AcctAPIGateWay/CardportalSecurityAPi/api/CardProductSetup"
          );
          ...
          const newData = json_object.map((x) => {
            return {
              productCode: product.cardProductCode.toString(),
              branchNo: company.data.branch.toString(),
              ...
              addressLine1: x.addressLine1.toString(),
              addressLine2: x.addressLine2.toString(),
              cityCode: x.cityCode.toString(),
              regionCode: x.regionCode.toString(),
              countryCode: x.countryCode.toString(),
              legalID: x.legalID.toString(),
              idCardTypeCode: x.idCardTypeCode.toString(),
              documentIssueDate: x.documentIssueDate.toString(),
              expiryDateOfDoc: x.expiryDateOfDoc.toString(),
              accountNbr: x.accountNbr.toString(),
              nameOnCard: x.nameOnCard.toString(),
              socioProfCode: x.socioProfCode.toString(),
            };
          });
          ...
          const newSorted = sorted_arr2.map((s) => {
            return {
              documentIssueDate: new Date(s.documentIssueDate)
                .toISOString()
                .substr(0, 10),
              expiryDateOfDoc: new Date(s.expiryDateOfDoc)
                .toISOString()
                .substr(0, 10),
            };
          });
            ...
            if (valResults2.length <= 0) {
              try {
                ...
                const response = await axios.post(
                  "https://acctgw.zenithbank.com/AcctAPIGateWay/CardportalOperation/api/CardRequest/makecardrequest",
                  formData,
                  {
                    headers: {
                ...
                if (response.data[0].responseCode == "00") {
                  ...
                  document.getElementById("bulk-status-success").style.display =
                    "flex";
                } else {
                  console.log("400");
                  document.getElementById("bulk-loader").style.display = "none";
                  document.getElementById("bulk-status-failed").style.display =
                    "flex";
                }
              } catch (error) {
                console.log("500");
              ...
              document.getElementById("error-text").innerHTML =
                "Expiry Date is less than Issue Date";
            }
          } else {
            document.getElementById("bulk-status-failed").style.display =
              "flex";
            document.getElementById("error-text").innerHTML =
              "Multiple account number instants";
          }
        });
      };
    ...
    async sendRequest() {
      ...
      const formData = {
        ...
        documentIssueDate: moment(this.form.documentIssueDate).format(
          "DD-MM-YYYY"
        ),
        expiryDateOfDoc:
          this.form.idCardTypeCode == "02" || this.form.idCardTypeCode == "04"
            ? "06-10-3021"
            : moment(this.form.expiryDateOfDoc).format("DD-MM-YYYY"),
      ...
      try {
        const response = await axios.post(
          this.getUrl2 + "api/CardRequest/makecardrequest",
          [formData]
        );
        if (response.data[0].responseCode == "00") {
          this.loader = false;
