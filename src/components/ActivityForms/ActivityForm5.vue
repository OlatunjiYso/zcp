<template>
 ...
 <div class="content-header">Default Pin Generation</div>
      <div class="content-sub">Search and generate a default pin for a card</div>
      <div>
        <div>
           ...
           <div v-else>
                    ...
                    <thead>
                        ...
                        <tbody>
                            ...
                            <div @click="Generate(result)" style="cursor:pointer" class="table-btn">Generate<span class="table-button-icon"></span></div>
                            </td> 
                        </tr>
   ...
   this.cardData = response.data
       ...
       const user = JSON.parse(localStorage.getItem("user-mfb"))
         ...
         try {
             ...
             const response = await axios.post(this.getUrl2 + 'api/CardPin/pinretrieval',form)
             if(response.data.responseCode == "00"){
                this.cardData = []
               this.loader = false;
               this.status = true;
               this.state = 'success';
               this.message = 'Pin generated Successfully'
             }
             else{
               this.loader = false;
               this.status = true;
