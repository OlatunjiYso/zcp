          ...
          <tbody>
            ...
            >
              ...
              <td class="app-table2-data">
                <div
                  @click="CheckStatus(result)"
                  style="cursor: pointer"
                  class="table-btn"
                >
                  Card Status<span class="table-button-icon"></span>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    async searchForCard(e) {
      ...
      if (e.keyCode === 13) {
        ...
        const response = await axios.get(
          this.getUrl2 +
            "api/CardRequest/cardbyaccountNo/" +
            user.companyId +
            "/" +
            this.accountNbr
        );
        if (response.status == 200) {
          const requests = response.data.map((x) => {
            return {
              ...x,
              productCode:
                this.getCardSetup.length > 0
                  ? this.getCardSetup.find((entry) => {
                      return x.productCode === entry.cardProductCode;
                    }).description
                  : null,
            };
          });
      ...
      if (result.clientCode.length > 3) {
        ...
        try {
          const response = await axios.post(
            this.getUrl2 + "api/CardRequest/cardstatus",
            form
          );
