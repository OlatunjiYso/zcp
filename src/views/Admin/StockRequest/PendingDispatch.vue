<template>
  <div>
    ...
    />
    <div class="content-header">Card Stock Requests Pending Dispatch</div>
    <div class="content-sub">Here are the cardstocks that needs to be</div>
  ...
  methods: {
    ...
    async selectAll() {
      ...
      if (checkbox.checked == true) {
        for (var i = 0; i < this.reqPendingDispatch.length; i++) {
          this.newReq.push(parseInt(this.reqPendingDispatch[i].id));
          document.getElementById(
            `ReqA${this.reqPendingDispatch[i].id}`
          ).checked = true;
        }
        this.checkBtnState();
      } else {
