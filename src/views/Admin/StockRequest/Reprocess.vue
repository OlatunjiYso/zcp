          ...
          <div>
            ...
            <form @submit.prevent="sendRequest">
              <div class="form-flex">
                ...
                <div class="form-flex-col-3">
                  <label class="login-label"
                    >Type of Card<span style="color: red">*</span></label
                  >
                  <select
                    required
                  ...
                  >
                    <option
                      v-for="(result, index) in reasons"
                      :key="index"
                      :value="result.reasonId"
                    >
  ...
  methods: {
    ...
    async sendRequest() {
      ...
      const formData = {
        ...
        documentIssueDate: moment(this.requestData.documentIssueDate).format(
          "DD-MM-YYYY"
        ),
        expiryDateOfDoc:
          this.requestData.idCardTypeCode == "02" ||
          this.requestData.idCardTypeCode == "04"
            ? "06-10-3021"
            : moment(this.requestData.expiryDateOfDoc).format("DD-MM-YYYY"),
      ...
      try {
        const response = await axios.post(
          this.getUrl2 + "api/CardRequest/ReprocessrejectedRequest",
          formData
        );
