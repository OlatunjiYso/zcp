...
export default createStore({
  ...
  mutations: {
    ...
    setAccountHolders(state, payload) {
  ...
  actions: {
    ...
    async getAccountHolders({ commit, state }, companyId) {
      const result = await axios.get(state.url2 + 'api/AccountHolders/' + companyId,
        {
          headers: {
            "Content-Type": "application/json"
          }
      ...
      commit('setAccountHolders', result.data);
      return result;
    },
